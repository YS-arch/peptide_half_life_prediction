...
def getGlobalDescriptor(seqList, amide):
	...
	for i, seq in enumerate(seqList):
		des = GlobalDescriptor(seq)
		# 这里C末端酰胺化需要设置
		des.calculate_all(amide=amide[i])
		desT = des.descriptor.T
		for i, name in enumerate(des.featurenames):
			res["modlAMP" + name].append(desT[i])  # 'MW'会有歧义
	return res
...
"""
key error
CC[C@H](C)[C@H](NC([C@H](C(C)C)NC([C@@H](NC([C@@H](NC([C@@H](NC(CNC(CN(C)C(CCC(S[*][Co])=O)=O)=O)=O)C(C)C)=O)[C@@H](C)CC)=O)[C@@H](C)O)=O)=O)C(N[C@@H](CCCNC(N)=N)C(N1CCC[C@H]1C(NCC)=O)=O)=O
s[*] -> mouse bolld modification 
"""
...
def _getPymoleculeDescriptorFromSmi(smi):
	...
	# casuse nan
	# res.update(charge.GetCharge(mol))		#charge.py:612: RuntimeWarning: invalid value encountered in double_scalars
	# res.update(connectivity.GetConnectivity(mol))  #connectivity.py:1019: RuntimeWarning: divide by zero encountered in double_scalars
	...
	# res.update(moreaubroto.GetMoreauBrotoAuto(mol))#key error '*'
	...
	# res.update(bcut.GetBurden(mol))	#key error '*'
...
def _getPyproteinDescriptorFromSeq(seq):
	...
	# res.update(protein_class.GetDPComp())  # 400		#除0bug
