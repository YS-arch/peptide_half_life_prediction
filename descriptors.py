...
def _getPymoleculeDescriptorFromSmi(smi):
	...
	res.update(geary.GetGearyAuto(mol))
...
def _getPyproteinDescriptorFromSeq(seq):
	...
	res.update(protein_class.GetDPComp())  # 400
	# res.update(protein_class.GetTPComp())               #8000
	res.update(protein_class.GetMoreauBrotoAuto())  # 240
	res.update(protein_class.GetMoranAuto())  # 240
	res.update(protein_class.GetGearyAuto())  # 240
	res.update(protein_class.GetCTD())  # 147
	# res.update(protein_class.GetPAAC())                 #除0bug
	# res.update(protein_class.GetAPAAC())                #除0bug
	res.update(protein_class.GetSOCN())  # 90
	res.update(protein_class.GetQSO())  # 100
...
def getMoleculeDesc(seqList, smiList, acetylation=None, amidation=None):
	...
	res.update(getGlobalDescriptor(seqList, amidation)) #10

	res.update(getPymoleculeDescriptorFromSmi(smiList)) #753

	return res                                          #765=10+753+2


def getMoleculeAndPeptideDesc(seqList, smiList, acetylation=None, amidation=None):
	res = getMoleculeDesc(seqList, smiList, acetylation, amidation) 
	res.update(getPyproteinDescriptorFromSeq(seqList))       #1820       
	return res                                               #2585=1820+765




def getAllDesc(seqList, smiList, acetylation=None, amidation=None):
	res = getMoleculeAndPeptideDesc(seqList, smiList, acetylation, amidation)   #2585
	res.update(getEnzyData(seqList))                                            #39
	return res                                                                  #2624=2585+39
