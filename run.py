...
descs = ['MoleculeDesc', 'MoleculeAndPeptideDesc', 'AllDesc']

tasks = ['Human_blood_modification', 'Human_blood_nature', 'mouse_blood_modification', 'mouse_blood_nature',
...
models = {linear_model.LinearRegression: 'LR', linear_model.Perceptron: "Perception",
		  ...
		  ensemble.RandomForestRegressor: 'RF', ensemble.GradientBoostingRegressor: 'GBT',
...
for task in tasks:
	for desc in descs:
		try:
			...
			df = readFileAndCal(path='./data/' + task + '.csv', mode='get' + desc, before_num=10, before_open=False)
			print("{}_{} done!,shape={}".format(task, desc, df.shape))
		...
		for model_class in models.keys():
			...
			met.to_csv("./metrics/{}_{}_{}.csv".format(task, desc, models[model_class]))
