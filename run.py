...
result = pd.DataFrame(
	columns=['desc', 'task', 'models', 'transform', 'modes', 'r', 'r2', 'mae', 'mse', 'rmse', 'cross_val_score'])
...
for task in tasks:
	for desc in descs:
		...
		for transform_str, transform_fn in transform.items():
			...
			x_train, y_train, x_test, y_test = data.get_spxy_data(test_size)
			...
			for model_str, getModel in models.items():
				for mode in modes:
					...
					score = model_selection.cross_val_score(model, x_train, y_train, cv=5, scoring='r2')
					...
					result.loc[len(result)] = [desc, task, model_str, transform_str, mode, r, r2, mae, mse, rmse,str(score)]
